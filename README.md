...
Умное предприятие - это комплексная система, которая обеспечивает комфорт и бесперебойность производственных процессов, мониторинг состояния производства, безопасность, экономию ресурсов в автоматическом режиме.
...
- Нет блокировки на производители специализированной автоматики для:
  ...
  - Так как автоматизация строится на универсальных устройствах Wirenboard, и [Zigbee](https://www.zigbee2mqtt.io/supported-devices).
    ...
    - Да, `НЕ` реализованы все возможные случаи, но если ваш случай `НЕ` реализован, то мы можете обратиться к нам, и мы соберем схему на железе, реализуем и отладим ПО с учетом всех частных случаев, и добавим такую автоматизацию в нашу коллекцию, и все последующие инсталляции такой инженерной системы будут происходить без отладки ПО.
...
- Вентиляция
  ...
  - Подогрев воздуха
    ...
    - Через отопление
      ...
      - Управление насосом
      - Управление автоматизацией `отопление`, запрос нужной температуры
  - Увлажнение воздуха
...
- Безопасность
  ...
  - Контроль движения
  - Уведомления
  - Управление автоматизацией `освещение`, включение той или иной зоны освещения
- Учет
  - Учет воды
  - Учет газа
  - Учет расхода тепла
...
- Источник бесперебойного питания
  ...
  - Управление автоматизацией `Автоматический ввод резерва`, запуск генератора
...
Module основан на универсальной автоматике от компании Wirenboard + [устройствах](https://www.zigbee2mqtt.io/supported-devices/) работающих по протоколу Zigbee.
...
- [WB 8](https://wirenboard.com/ru/product/wiren-board-8)
...
1. База данных.
2. К процесс в котором запущен код Module который выполнят все необходимые вычисления и отдает команды контроллеру.
3. Фронтенд для пользовательского взаимодействия.

- Устанавливаем `Module` на ту же виртуальную машину.
- Прописываем у него в переменных окружения данные для подключения к нашему MQTT брокеру.
- Входим в web интерфейс `Module` (адрес будет похож на `http://CLOUD_HOST`), и смотрим вкладку `Устройства`, там должны появиться все устройства которые имеются в `Wirenboard Web GUI`.
