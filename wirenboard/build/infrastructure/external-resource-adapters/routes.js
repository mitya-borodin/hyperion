...
const resetRoutes = async () => {
    try {
        ...
        if (currentRoutes.stdout.includes('default via 192.168.1.1 dev eth0')) {
            const { stdout, stderr } = await (0, execa_1.default)('ip', ['route', 'del', 'default', 'via', '192.168.1.1']);
            result.push(stdout, stderr);
        }
        if (currentRoutes.stdout.includes('default via 192.168.0.100 dev usb0')) {
            const { stdout, stderr } = await (0, execa_1.default)('ip', ['route', 'del', 'default', 'via', '192.168.0.100']);
            result.push(stdout, stderr);
        }
        const addEth = await (0, execa_1.default)('ip', ['route', 'add', 'default', 'via', '192.168.1.1', 'metric', '1']);
        const addUsb = await (0, execa_1.default)('ip', ['route', 'add', 'default', 'via', '192.168.0.100', 'metric', '2']);
...
const removeEthRoute = async () => {
    try {
        ...
        if (currentRoutes.stdout.includes('default via 192.168.1.1 dev eth0 metric 1')) {
            ...
            const addEth = await (0, execa_1.default)('ip', ['route', 'add', 'default', 'via', '192.168.1.1', 'metric', '3']);
