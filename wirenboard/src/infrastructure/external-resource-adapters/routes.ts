...
export const resetRoutes = async () => {
  try {
    ...
    if (currentRoutes.stdout.includes('default via 192.168.1.1 dev eth0')) {
      const { stdout, stderr } = await execa('ip', ['route', 'del', 'default', 'via', '192.168.1.1']);

      result.push(stdout, stderr);
    }

    if (currentRoutes.stdout.includes('default via 192.168.0.100 dev usb0')) {
      const { stdout, stderr } = await execa('ip', ['route', 'del', 'default', 'via', '192.168.0.100']);

      result.push(stdout, stderr);
    }

    const addEth = await execa('ip', ['route', 'add', 'default', 'via', '192.168.1.1', 'metric', '1']);

    const addUsb = await execa('ip', ['route', 'add', 'default', 'via', '192.168.0.100', 'metric', '2']);
...
export const removeEthRoute = async () => {
  try {
    ...
    if (currentRoutes.stdout.includes('default via 192.168.1.1 dev eth0 metric 1')) {
      ...
      const addEth = await execa('ip', ['route', 'add', 'default', 'via', '192.168.1.1', 'metric', '3']);
